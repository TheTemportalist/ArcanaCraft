...
public class BindRecipes {
	...
	public static enum Type {
		QUOM, ITEM_BINDER, QUOM_BINDER, FLUID_BINDER
	}
	
	public static class BindRecipe {
		
		public final BindRecipes.Type	bindType;
		public final List<ItemMeta>		inputs;
		public final ItemMeta			output;
		
		public BindRecipe(BindRecipes.Type type, ItemMeta... itemMetas) {
			this.bindType = type;
				...
				if (this.inputs.size() == that.inputs.size()) {
					if (this.output.equals(that.output)) {
						for (int i = 0; i < this.inputs.size(); i++) {
							if (!this.inputs.get(i).equals(that.inputs.get(i))) return false;
						}
						return true;
					}
				}
	...
	public static void registerRecipes() {
		...
		recipes.addRecipe(new BindRecipe(BindRecipes.Type.QUOM, new ItemMeta(Blocks.iron_block, 0),
				new ItemMeta(ACItems.focusBasic, 0), new ItemMeta(ACBlocks.bindableCore, 0)));
	...
	public ItemStack getRecipeOutput(BindRecipes.Type bindType, ItemStack[] inputs) {
		List<BindRecipe> recipes = BindRecipes.getRecipeList();
		for (int i = 0; i < recipes.size(); i++) {
			if (recipes.get(i).bindType == bindType && recipes.get(i).hasInputs(inputs)) {
